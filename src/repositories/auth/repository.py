...
class TokenRepository:
    ...
    async def verify_user_token(cls, token: str) -> VerificationResult:
        payload = inh_accounts.decode_token(token)
        if payload is None:
            return VerificationResult(success=False)
        innohassle_id: str | None = payload.innohassle_id
        user_data = await inh_accounts.get_user(innohassle_id=innohassle_id)
        if user_data is None:
            return VerificationResult(success=False)
        return VerificationResult(
            success=True,
            telegram_id=user_data.telegram_info.id if user_data.telegram_info else None,
            email=user_data.innopolis_info.email if user_data.innopolis_info else None,
