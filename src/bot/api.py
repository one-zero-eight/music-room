...
class InNoHassleMusicRoomAPI:
    ...
    def _auth_session(self, session: aiohttp.ClientSession, telegram_id: int) -> None:
        session.headers.update({"Authorization": f"Bearer {telegram_id}:{bot_settings.bot_token.get_secret_value()}"})
...
client: InNoHassleMusicRoomAPI = InNoHassleMusicRoomAPI(bot_settings.api_url)
