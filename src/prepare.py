...
def check_and_prompt_bot_token(use_bot_settings: bool):
    """
    Ensure the token is set in `settings.yaml`. If missing, request token from user.
    """
    settings = get_settings()
    if use_bot_settings:
        bot_token = settings.get("bot_settings", {}).get("bot_token")
    else:
        bot_token = settings.get("api_settings", {}).get("bot_token")
...
def check_and_generate_secret_api_key():
    """
    Ensure the api_key is set in `settings.yaml`. If missing, generate random_one
    """
    settings = get_settings()
    api_key = settings.get("api_settings", {}).get("api_key")

    if not api_key or api_key == "...":
        print("⚠️ `api_key` is missing in `settings.yaml`.")
        print("  ➡️ Generate a random one")
        secret = secrets.token_hex(32)
        try:
            with open(SETTINGS_FILE) as f:
                as_text = f.read()
            as_text = as_text.replace("api_key: null", f"api_key: {secret}")
            as_text = as_text.replace("api_key: ...", f"api_key: {secret}")
            with open(SETTINGS_FILE, "w") as f:
                f.write(as_text)
